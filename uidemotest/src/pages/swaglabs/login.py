...
class LoginPage(LoginPageLocator):
    ...
    def is_login_page_displayed(self):
        username_field_displayed = self.sl.wait_get_element(self.USER_NAME_FIELD)
        password_field_displayed = self.sl.get_element(self.PASSWORD_FIELD).is_displayed()
        login_button_displayed = self.sl.get_element(self.LOGIN_BUTTON).is_displayed()

        return username_field_displayed and password_field_displayed and login_button_displayed

    def enter_username(self, username):
        self.sl.wait_and_input_text(self.USER_NAME_FIELD, username)
        return self
    ...
    def press_login_button(self):
        self.sl.get_element(self.LOGIN_BUTTON).click()

    def __get_accepted_usernames(self):
        usernames_text = self.sl.get_text(self.ACCEPTED_USERNAMES)
    ...
    def get_password(self):
        text = self.sl.get_text(self.PASSWORD)
        password = text.split('\n')[1]

        return password

    def get_error_message(self):
        return self.sl.get_text(self.ERROR_MESSAGE)

    def get_password_input_field_type_value(self):
        password_input_field = self.sl.get_element(self.PASSWORD_FIELD)
